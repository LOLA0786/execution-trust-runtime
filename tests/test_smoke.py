...
def test_smoke_imports():
    ...
    from core.vault.private_vault import vault, PrivateVault
    from core.llm.grok_client import grok_client
    from agents.base_agent import BaseAgent
    from integrations.firewalled import salesforce, salesforce_client
    assert vault is not None
    assert grok_client is not None
    assert BaseAgent is not None
    assert salesforce is not None
    assert salesforce_client is not None
